...
async def show_connection(message: Message):
    ...
    text = (
        f"🔗 <b>Доступ к материалам проекта</b>\n\n"
...
async def send_qr_code(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer_photo(
            photo=photo,
            caption="📱 Отсканируйте QR код для доступа к материалам"
        )
...
async def show_instructions(callback: CallbackQuery):
    ...
    text = (
        "📖 <b>Инструкции по доступу к материалам</b>\n\n"
        "<b>1. Скачайте клиент для  вашей ОС:</b>\n"
        "• <b>Windows (10/11):</b>\n"
        "   - Hiddify ← самый простой для новичков\n"
        "     Скачать: https://hiddify.com\n"
        "     Как подключить: Открой → «+» → «Import from clipboard» или «Scan QR» → вставь ссылку от бота → Подключить.\n"
        "   - NekoBox / NekoRay (гибкий, открытый код)\n"
        "     Скачать: https://getnekobox.com/en/\n"
        "   - InvisibleMan-XRay (лёгкий, минималистичный)\n"
        "     Скачать: https://invisiblemanvpn.github.io/\n"
        "   - Clash for Windows (для продвинутых)\n"
        "     Скачать: https://dl.p6p.net/clash_for_windows/\n\n"
        "• <b>Android:</b>\n"
        "   - Hiddify ← самый популярный и стабильный\n"
        "     Скачать: https://hiddify.com (Google Play или прямой apk)\n"
        "   - sing-box (если уже привык к Clash-подобным клиентам)\n\n"
        "• <b>iOS / iPadOS:</b>\n"
        "   - Streisand ← лучший современный интерфейс\n"
        "     App Store: https://apps.apple.com/ru/app/streisand/id6450534064\n"
        "   - v2rayTUN\n"
        "     App Store: https://apps.apple.com/ru/app/v2raytun/id6476628951\n"
        "   - Shadowrocket (классика, платный)\n"
        "   - Quantumult X (для опытных)\n\n"
        "• <b>macOS:</b>\n"
        "   - Streisand (только Apple Silicon M1/M2/M3/M4) через App Store.\n"
        "   - sing-box (работает на Intel и Apple Silicon)\n"
        "   - ClashX (полный контроль)\n\n"
        "• <b>Linux:</b>\n"
        "   - NekoRay (есть версия для Linux)\n"
        "   - v2rayA (очень удобный веб-интерфейс)\n"
        "   - sing-box + графическая оболочка (например, sing-box GUI)\n"
        "   - Clash for Linux / Clash Meta\n\n"
        "<b>2. Установите и откройте приложение</b>\n\n"
        "<b>3. Добавьте подписку:</b>\n"
        "   • Найдите кнопку 'Добавить подписку', 'Import from URL', 'Import from clipboard' или аналогичную\n"
        ...
        "   • Включите доступ к материалам (переключите тумблер)\n"
        "   • Дождитесь установки соединения\n\n"
        "<b>🔧 Устранение неполадок:</b>\n"
        "• <b>Нет подключения:</b> Проверьте интернет, перезапустите клиент доступа, обновите подписку\n"
...
async def show_help(message: Message):
    ...
    text = (
        ...
        "<b>📱 Рекомендуемые клиенты для доступа к материалам:</b>\n"
        "• <b>Windows (10/11):</b> Hiddify (самый простой для новичков), NekoBox/NekoRay, InvisibleMan-XRay, Clash for Windows (для продвинутых)\n"
        "• <b>Android:</b> Hiddify (самый популярный и стабильный), sing-box\n"
        "• <b>iOS/iPadOS:</b> Streisand (лучший современный интерфейс), v2rayTUN, Shadowrocket (классика, платный), Quantumult X (для опытных)\n"
        "• <b>macOS:</b> Streisand (только Apple Silicon), sing-box (работает на Intel и Apple Silicon), ClashX (полный контроль)\n"
        "• <b>Linux:</b> NekoRay, v2rayA (удобный веб-интерфейс), sing-box + графическая оболочка, Clash for Linux / Clash Meta\n\n"
        "<b>💡 Полезные советы:</b>\n"
        "1. Для подключения используйте ссылку доступа или QR код из раздела 'Моя подписка' → 'Настройка подключения'.\n"
        "2. Уведомления об истечении доступа можно настроить в меню '⚙️ Настройки'.\n"
        "3. При проблемах с подключением перезапустите клиент доступа и обновите подписку.\n"
        "4. Тестовый доступ предоставляется на 7 дней с ограниченным трафиком.\n"
        "5. Для экономии трафика отключайте доступ при загрузке больших файлов или просмотре локального контента.\n"
        "6. Используйте режим 'Split Tunneling' (если поддерживается клиентом) для выбора приложений, работающих через доступ к материалам.\n"
        "7. Регулярно проверяйте обновления клиента доступа для улучшения безопасности и скорости.\n"
        "<b>🔒 Безопасность:</b>\n"
        "• Не передавайте свою ссылку доступа третьим лицам\n"
        "• Регулярно обновляйте пароли и используйте двухфакторную аутентификацию где возможно\n"
        "• При использовании публичных Wi-Fi всегда включайте доступ к материалам\n\n"
        "<b>Вопросы?</b>\n"
